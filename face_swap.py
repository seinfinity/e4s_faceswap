...
# scr module error --> src 파일을 scripts 파일 안으로 위치 이동 
...
def faceSwapping_pipeline(source, target, opts, save_dir, target_mask=None, need_crop = False, verbose = False, only_target_crop=False):
    ...
    ##########################추가 -- saved cropped images##########################
    if need_crop or only_target_crop:
        crops[0].save(os.path.join(save_dir, f"{source_and_target_files[0][0]}_source_cropped.png"))
        crops[1].save(os.path.join(save_dir, f"{source_and_target_files[1][0]}_target_cropped.png"))
    ################################################################################
...
if __name__ == "__main__":
    ...
    faceSwapping_pipeline(opts.source, opts.target, opts, save_dir=opts.output_dir, 
                          target_mask = target_mask_seg12, need_crop = opts.need_crop, verbose = opts.verbose) 
